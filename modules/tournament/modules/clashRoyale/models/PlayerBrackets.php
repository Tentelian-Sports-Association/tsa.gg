...
class PlayerBrackets extends ActiveRecord
{
    ...
    {
        return 'clashRoyale_player_brackets';
    }
    ...
    */
    ...
    public function getEncounterData($participant_id)
    {
        return PlayerBracketEncounter::find()->where(['id' => $this->encounter_id])->andWhere(['tournament_id' => $this->tournament_id])->andWhere([])->andWhere(['player_id' => $participant_id])->orderBy(['game_round' => SORT_ASC])->all();
	}

    public function getEncounterParticipant1($round)
    {
        return PlayerBracketEncounter::find()->where(['id' => $this->encounter_id])->andWhere(['tournament_id' => $this->tournament_id])->andWhere(['game_round' => $round])->andWhere(['player_id' => $this->participant_1])->one();
	}

    public function getEncounterParticipant2($round)
    {
        return PlayerBracketEncounter::find()->where(['id' => $this->encounter_id])->andWhere(['tournament_id' => $this->tournament_id])->andWhere(['game_round' => $round])->andWhere(['player_id' => $this->participant_2])->one();
	}
    ...
    {
        /** Umändern, sodass der gewinner immer im oberren bracket steht und sich dadurch die Farbe auch ändert, wie in RL */
	...
	public static function clearForTournament($tournament_id)
    {
        ...
        $encounters = PlayerBracketEncounter::getAlllByTournament($tournament_id);
	...
	public function moveParticipantsNextRound($winnerNumber) {
		...
		$winnerBracket->setParticipantByBackRef(($winnerNumber == 1)? $this->getPlayerParticipant1Id() : $this->getPlayerParticipant2Id(), $this->getId());
		if ($looserBracket) {
			$looserBracket->setParticipantByBackRef(($winnerNumber == 1)? $this->getPlayerParticipant2Id() : $this->getPlayerParticipant1Id(), $this->getId());
		}
	}
	...
	public function setParticipantByBackRef($id, $preBracketId) {
        ...
        for($i = 0; $i < $this->getBestOf(); $i++)
        {
            ...
            {
                $encounterParticipant1 = $this->getEncounterParticipant1($i+1);
            ...
            {
                $encounterParticipant2 = $this->getEncounterParticipant2($i+1);
			...
			foreach ($players_left as $key => $player) {
                if($player)
                {
				    if ($player->getId() == $encounter->getPlayerId()) {
					    ...
					    $leftGoals[$encounter->game_round] += $encounter->crowns;
				    }
                }
			}
            ...
            if($player)
                {
				    if ($player->getId() == $encounter->getPlayerId()) {
					    ...
					    $rightGoals[$encounter->game_round] += $encounter->crowns;
				    }
                }
			}
		}
    ...
    public function getWins()
    {
		...
		foreach ($encounters as $key => $encounter)
        {
			if ($players_left && $players_left->getId() == $encounter->getPlayerId())
            {
				$leftGoals[$encounter->getGameRound()] = $encounter->getCrowns();
			}            

			if ($players_right && $players_right->getId() == $encounter->getPlayerId()) {
				$rightGoals[$encounter->getGameRound()] = $encounter->getCrowns();
			}
		}
